...
!developmentChains.includes(network.name)
    ...
    : describe("Dynamic SVG NFT Unit Tests", async function () {
          ...
          describe("Constructor", async function () {
              it("initlizes the contract correctly", async function () {})
          })
          describe("Mint NFT", async function () {
              it("reverts if value is lower than mintFee", async function () {
                  const fee = await dynamicNft.getMinFee()
                  await expect(
                      dynamicNft.requestNft({
                          value: fee.sub(ethers.utils.parseEther("0.001")),
                      })
                  ).to.be.revertedWith("DynamicNFT__NotEnoughETH")
              })
